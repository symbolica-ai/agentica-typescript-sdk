...
import { isAgentCallMethodReference, isAgenticaLibraryFunction } from '@transformer/transformer-utils';
...
export type ProcessedRecord = {
    ...
    agenticArgumentPath?: string[]; // Accessor path for symbols explicitly passed to agentic(), e.g. magic({ foo.bar })
    agenticArgumentExpr?: ts.Expression; // Original explicit expression, preferred for codegen
...
export function upsertContextFromTraversalRecords(
    ...
    for (const [uid, record] of traversal.recordsByUid) {
        ...
        const systemInternalType =
            isRuntimeKind && !modulePath && !record.__agenticArgumentPath && !!accessorPath?.length; // Has computed path (e.g., ["Function", "prototype", "apply"])
        ...
        const rec: ProcessedRecord = {
            ...
            agenticArgumentPath: record.__agenticArgumentPath,
            agenticArgumentExpr: (record as any).__agenticArgumentExpr,
...
function validateScopeExpression(expr: ts.Expression, checker: ts.TypeChecker, call: ts.CallExpression): void {
    if (isAgenticaLibraryFunction(expr, call.getSourceFile())) {
        const sourceFile = call.getSourceFile();
        const lineNumber = sourceFile.getLineAndCharacterOfPosition(expr.getStart()).line + 1;
        throw new Error(
            `Cannot pass 'spawn', 'agentic', or 'agenticPro' functions as scope arguments at ${sourceFile.fileName}:${lineNumber}. ` +
