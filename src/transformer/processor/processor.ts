...
import { isCallMethod, isMagicFunCall, isSpawnFunCall } from '@transformer/transformer-utils';
...
export type MagicFunSite = {
    ...
    functionName?: string;
    docString?: string;
    siteType: 'magic' | 'spawn' | 'call';
};

export function collectMagicFunSites(program: ts.Program): MagicFunSite[] {
    const logger = createConsolaLogger('collector');
    const checker = program.getTypeChecker();
    const sites: MagicFunSite[] = [];
    let idCounter = 0;
    ...
    logger.debug(`Collected ${sites.length} magic sites across ${program.getSourceFiles().length} files`);
    return sites;
}

export type MagicSiteContext = {
...
export function generateContextAtMagicSite(
    checker: ts.TypeChecker,
    program: ts.Program,
    site: MagicFunSite
): MagicSiteContext {
    const logger = createConsolaLogger('processor').withScope(`site-${site.id}`);
    const span = logger.startSpan(`generateContextAtMagicSite-${site.id}`);
