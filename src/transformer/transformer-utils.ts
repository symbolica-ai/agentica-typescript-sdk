...
export function isAgenticFunCall(node: ts.Node): node is ts.CallExpression {
    ...
    if (ts.isCallExpression(node) && ts.isTaggedTemplateExpression(node.expression)) {
        ...
        return checkAgenticaImport(node.getSourceFile(), calleeName, isAgenticFnModule);
    }
...
function checkAgenticaImport(sf: ts.SourceFile, calleeName: string, moduleCheck: (spec: string) => boolean): boolean {
...
function checkAgentReceiver(receiver: ts.Expression, checker: ts.TypeChecker): boolean {
    ...
    const isAgentLike = (t: ts.Type): boolean => {
        ...
        for (const decl of declarations) {
            ...
            if (sourceFile && isAgentModule(sourceFile.fileName)) {
                return true;
            }
        }
        return false;
...
export function addAgenticTransformationImport(factory: ts.NodeFactory, sourceFile: ts.SourceFile): ts.SourceFile {
    const visitor = (node: ts.Node): ts.Node => {
        if (ts.isImportDeclaration(node)) {
            ...
            const hasAgentic = clause.namedBindings.elements.some(
                (e) => e.name.text === 'agentic' || e.name.text === 'agenticPro' || e.name.text === 'spawn'
            );
            if (!hasAgentic) return node;

            const mod = node.moduleSpecifier;
            if (ts.isStringLiteral(mod)) {
                const spec = mod.text;
                if (isAgenticFnModule(spec) || isAgentModule(spec)) {
                    const hasAgenticTransformation = clause.namedBindings.elements.some(
                        (e) => e.name.text === AGENTIC_TRANSFORMATION_NAME
                    );
                    ...
                    if (isAgenticFnModule(spec) && !hasAgenticTransformation) {
                        newElements.push(
                            factory.createImportSpecifier(
                                false,
                                undefined,
                                factory.createIdentifier(AGENTIC_TRANSFORMATION_NAME)
                            )
                        );
                    }

                    if (isAgentModule(spec) && !hasSpawnTransformation) {
                        newElements.push(
                            factory.createImportSpecifier(
                                false,
                                undefined,
...
function isAgenticaStdModule(spec: string): boolean {
    return spec.includes('agentica/std');
}

function isAgenticFnModule(spec: string): boolean {
    return (spec.includes('agentica/agentic') || spec.includes('symbolica/agentica')) && !isAgenticaStdModule(spec);
}

function isAgentModule(spec: string): boolean {
    ...
    return (
        ...
        !isAgenticaStdModule(spec)
    );
}

export function isAgenticaLibraryModule(filePath: string): boolean {
    return (isAgenticFnModule(filePath) || isAgentModule(filePath)) && !isAgenticaStdModule(filePath);
}

export function isAgenticaLibraryFunction(node: ts.Expression, sourceFile: ts.SourceFile): boolean {
...
export function createAgenticIdDeclaration(factory: ts.NodeFactory, id: number): ts.VariableStatement {
...
export function createAgenticOutputDeclaration(factory: ts.NodeFactory, outputType: DefMsg): ts.VariableStatement {
    return factory.createVariableStatement(
        undefined,
        factory.createVariableDeclarationList(
            [
...
export function createAgenticPromptDeclaration(
...
export interface WrapAgenticCallOptions {
    factory: ts.NodeFactory;
...
function extractArgumentsFromCall(
    ...
    if (ts.isTaggedTemplateExpression(call.expression)) {
        ...
        return {
            prompt: factory.createIdentifier('__AGENTIC_PROMPT__'),
            config,
        };
    }
...
function createAgenticContextObject(factory: ts.NodeFactory, docString?: string): ts.ObjectLiteralExpression {
    return factory.createObjectLiteralExpression(
        [
            factory.createPropertyAssignment(
                factory.createIdentifier('concepts'),
                factory.createIdentifier('__AGENTIC_CONTEXT__')
            ),
            factory.createPropertyAssignment(
                factory.createIdentifier('siteId'),
                factory.createIdentifier('__AGENTIC_ID__')
            ),
            factory.createPropertyAssignment(
                factory.createIdentifier('siteOutputType'),
                factory.createIdentifier('__AGENTIC_OUTPUT__')
            ),
            factory.createPropertyAssignment(
...
function buildAgenticStatements(
    factory: ts.NodeFactory,
    dynamicImports: ts.Statement[],
    contextDecl: ts.Statement | undefined,
    siteId: number,
    ...
    statements.push(createAgenticIdDeclaration(factory, siteId));

    if (!isSpawn && siteOutputType) {
        statements.push(createAgenticOutputDeclaration(factory, siteOutputType));
    }

    if (promptTemplate) {
        statements.push(createAgenticPromptDeclaration(factory, promptTemplate));
    }
...
export function wrapTransformationSite(options: WrapAgenticCallOptions): ts.Expression {
    const { factory, call, code, functionName = AGENTIC_TRANSFORMATION_NAME } = options;
    ...
    if (isSpawn) {
        ...
        const contextArg = createAgenticContextObject(factory, docString);
    ...
    const agenticStatements = buildAgenticStatements(
        factory,
        dynamicImports,
        contextDecl,
        siteId,
    ...
    const arrowFunction = factory.createArrowFunction(
        ...
        factory.createBlock(agenticStatements, true)
    );
