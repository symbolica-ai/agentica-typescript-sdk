...
export function getOrCreateMapType(ctx: TraversalCtx, keyUid: DefnUID, valueUid: DefnUID): RefMsg<ClassMsg> {
    ...
    */
    ...
    const mapMsg = SystemMagicContext.createMapClassMsg(World.Client, mapUid, keyUid, valueUid);
...
export function traverseThroughClass(
    ...
    // HACK: Stop traversing globalThis - treat as plain object
    ...
    */
    ...
    if (allBases.length > 0) {
        bases = allBases.map((baseType: ts.Type) => {
            ...
            if (baseName && !hasConstructor(baseType) && SystemMagicContext.isSystemResourceName(baseName)) {
                const sysRef = SystemMagicContext.getClassRefByName(World.Client, baseName);
