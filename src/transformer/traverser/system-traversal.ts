...
export function tryPrimitiveType(t: ts.Type, ctx: TraversalCtx): ClassMsg | undefined {
    ...
    const f = t.flags;
    if (f & ts.TypeFlags.String) return SystemContext.getClassMsgByName(World.Client, 'string')!;
    if (f & ts.TypeFlags.Number) return SystemContext.getClassMsgByName(World.Client, 'number')!;
    if (f & ts.TypeFlags.Boolean) return SystemContext.getClassMsgByName(World.Client, 'boolean')!;
    // NOTE: BooleanLiteral NOT handled here - becomes Literal type like StringLiteral and NumberLiteral
    if (f & ts.TypeFlags.Null) return SystemContext.getClassMsgByName(World.Client, 'null')!;
    if (f & ts.TypeFlags.Undefined) return SystemContext.getClassMsgByName(World.Client, 'undefined')!;
    if (f & ts.TypeFlags.Void) return SystemContext.getClassMsgByName(World.Client, 'void')!;
    if (f & ts.TypeFlags.ESSymbol || f & ts.TypeFlags.UniqueESSymbol) {
        ...
        return SystemContext.getClassMsgByName(World.Client, 'symbol')!;
    }
    if (f & ts.TypeFlags.BigInt || f & ts.TypeFlags.BigIntLiteral)
        return SystemContext.getClassMsgByName(World.Client, 'number')!;
    /*
    ...
    */
    if (f & ts.TypeFlags.NonPrimitive) return SystemContext.getClassMsgByName(World.Client, 'object')!;
    if (f & ts.TypeFlags.Any) return SystemContext.getClassMsgByName(World.Client, 'object')!;

    if (f & ts.TypeFlags.Never) return SystemContext.getClassMsgByName(World.Client, 'void')!;
...
export function tryPlainSystemType(t: ts.Type, ctx: TraversalCtx): ClassMsg | undefined {
    ...
    const msg = SystemContext.getClassMsgByName(World.Client, instName);
...
export function tryGenericSystemType(
    ...
    if (f & ts.TypeFlags.StringLiteral) {
        ...
        const literalMsg = SystemContext.createLiteralClassMsg(World.Client, uid, value, 'string');
    ...
    if (f & ts.TypeFlags.NumberLiteral) {
        ...
        const literalMsg = SystemContext.createLiteralClassMsg(World.Client, uid, value, 'number');
    ...
    if (f & ts.TypeFlags.BooleanLiteral) {
        ...
        const literalMsg = SystemContext.createLiteralClassMsg(World.Client, uid, isTrue, 'boolean');
    ...
    if (isEnum(t)) {
        ...
        if (enumDeclaration) {
            ...
            const enumMsg = SystemContext.createEnumClassMsg(World.Client, uid, keys, values, symbol?.getName());
    ...
    if (f & ts.TypeFlags.Union && !(f & ts.TypeFlags.Boolean)) {
        ...
        if (hasTrue && hasFalse) {
            ...
            const booleanPrimitive = SystemContext.getClassRefByName(World.Client, 'boolean')!;
    ...
    if (checker.isArrayType && checker.isArrayType(t)) {
        ...
        const elemUuid = elementT
            ...
            : SystemContext.getClassRefByName(World.Client, 'Object')!.uid;
        const arrayMsg = SystemContext.createArrayClassMsg(World.Client, uid, elemUuid) as any;
    ...
    if ((t as any).target && (t as any).target.objectFlags & ts.ObjectFlags.Tuple) {
        ...
        if (target.hasRestElement) {
            ...
            return SystemContext.createArrayClassMsg(World.Client, uid, elemUuid) as any;
        }
        ...
        const tupleMsg = SystemContext.createTupleClassMsg(World.Client, uid, elementUuids) as any;
    ...
    if ((t as any).target || (t as any).aliasSymbol) {
        ...
        if ((name === 'Promise' || name === 'PromiseLike') && args.length === 1) {
            ...
            const futureMsg = SystemContext.createFutureClassMsg(World.Client, uid, valueUuid) as any;
        ...
        if (name === 'Set' && args.length === 1) {
            ...
            const setMsg = SystemContext.createSetClassMsg(World.Client, uid, elemUuid) as any;
        ...
        if (name === 'Map' && args.length === 2) {
            ...
            const mapMsg = SystemContext.createMapClassMsg(World.Client, uid, keyUuid, valueUuid) as any;
        ...
        if (name === 'Record' && args.length === 2) {
            ...
            const recordMsg = SystemContext.createRecordClassMsg(World.Client, uid, indexSignature) as any;
