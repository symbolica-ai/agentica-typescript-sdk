...
import { isMagicLibraryModule } from '@transformer/transformer-utils';
...
export type TraversalRecord = {
    ...
    __magicArgumentPath?: string[]; // Only set for symbols explicitly passed to magic()
    __magicArgumentExpr?: ts.Expression; // Preferred: exact expression explicitly passed by user
    __isTopLevel?: boolean; // True if explicitly passed to magic() or is the output type
...
export class SchemaTraversal {
    ...
    startTraversalAtFunction(
        ...
        if (this.recordsByType.has(typeId)) {
            ...
            record.__isTopLevel = true; // Explicitly passed to magic()
            return record;
        }
        ...
        const traversalRecord: TraversalRecord = {
            uid,
            msg,
            __magicArgumentPath: accessorPath, // Explicitly passed by user
            __magicArgumentExpr: explicitExpr,
            __module: sourceFile as string,
            __isTopLevel: true, // Explicitly passed to magic()
        };
    ...
    startTraversalAtClass(
        ...
        traversalRecord.__magicArgumentPath = accessorPath; // Explicitly passed by user
        traversalRecord.__magicArgumentExpr = explicitExpr;
        traversalRecord.__module = sourceFile as string;
        traversalRecord.__isTopLevel = true; // Explicitly passed to magic()
    ...
    startTraversalAtObject(
        ...
        if (isEnum(varType)) {
            ...
            clsRecord.__magicArgumentPath = accessorPath;
            clsRecord.__magicArgumentExpr = explicitExpr;
        ...
        const traversalRecord: TraversalRecord = {
            msg,
            uid,
            __magicArgumentPath: accessorPath,
            __magicArgumentExpr: explicitExpr,
            __module: sourceFile,
            __isTopLevel: true,
        };
    ...
    traverseTSType(
        ...
        if (symbol?.declarations && symbol.declarations.length > 0) {
            ...
            if (sourceFile && isMagicLibraryModule(sourceFile.fileName)) {
        ...
        if (this.traversalDepth > this.MAX_DEPTH) {
            ...
            finalizedMsg = SystemMagicContext.createObjectClassMsg(World.Client, uid, this.checker.typeToString(t));
        ...
        if ((finalizedMsg as any).uid && SystemMagicContext.getNameByUID((finalizedMsg as any).uid) !== undefined) {
    ...
    traverseComplexTSType(
        ...
        if (callSigs.length) {
            ...
            return SystemMagicContext.createFunctionClassMsg(World.Client, uid, funcMsg);
        }
        ...
        if (f & ts.TypeFlags.TypeParameter) {
            ...
            const clsMsg = SystemMagicContext.createObjectClassMsg(World.Client, uid, this.checker.typeToString(t));
        ...
        if (f & ts.TypeFlags.Index) {
            ...
            return SystemMagicContext.getClassMsgByName(World.Client, 'string')!;
        }
        ...
        if (f & ts.TypeFlags.Object || sym) {
            ...
            if (symName && !hasCtor && !isGenericInterface && SystemMagicContext.isSystemResourceName(symName)) {
                const sysMsg = SystemMagicContext.getClassMsgByName(World.Client, symName)!;
            ...
            if (isConstructorType(t, this.checker)) {
                ...
                return SystemMagicContext.getClassMsgByName(World.Client, 'TYPE')!;
            }
