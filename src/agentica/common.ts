...
function varMapFromRuntimeBuffer(
    ...
    for (const [name, termMsg] of runtime.rootFrame.replBuffer) {
        ...
        if (resource !== undefined) validateFeature(resource, mode);
...
export async function createAgentEnvironment(
    ...
    const varMap = varMapFromRuntimeBuffer(runtime, {}, logger, mode, 'init-time');
...
export async function invokeAgent(
    ...
    validateReturnType(varMap['__return_type'], runtime.rootFrame.context, toolMode); // We don't currently allow returning interfaces

    varMap = varMapFromRuntimeBuffer(runtime, varMap, logger, toolMode, 'run-time');
...
/**
 * Control the maximum number of tokens an agent or magic function can generate.
 *
 * @class MaxTokens
 *
...
export class Usage {
     ...
     */
    static fromCompletions(usage: Record<string, number>, lastUsage?: Usage): Usage {
     ...
     */
    replace(other: { inputTokens?: number; outputTokens?: number; totalTokens?: number }): Usage {
        return new Usage(
