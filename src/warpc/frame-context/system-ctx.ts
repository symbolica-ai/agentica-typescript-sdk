...
export class SystemObjectContext extends ObjectContext {
    constructor(public readonly uidGenerator: UIDGenerator) {
        super(uidGenerator);
        for (const name of SystemMagicContext.getObjectNames()) {
            const uid = SystemMagicContext.getUIDByName(this.world, name);
            const realObj = orUndefined((_globalThis as any)[name], (globalThis as any)[name]);
            const msg = SystemMagicContext.getObjectMsgByName(this.world, name);
...
export class SystemClassContext extends ClassContext {
    constructor(public readonly uidGenerator: UIDGenerator) {
        super(uidGenerator);
        for (const name of SystemMagicContext.getClassNames()) {
            const uid = SystemMagicContext.getUIDByName(this.world, name);
            const ctor = orUndefined((_globalThis as any)[name], (globalThis as any)[name]);
            const msg = SystemMagicContext.getClassMsgByName(this.world, name);
...
export class SystemFunctionContext extends FunctionContext {
    constructor(public readonly uidGenerator: UIDGenerator) {
        super(uidGenerator);
        for (const name of SystemMagicContext.getFunctionNames()) {
            const uid = SystemMagicContext.getUIDByName(this.world, name);
            const fn = resolveGlobalPath(name);
            const msg = SystemMagicContext.getFunctionMsgByName(this.world, name);
